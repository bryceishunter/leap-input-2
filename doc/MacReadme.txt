...
To install on Mac OS X with the .zip distribution (first seen in 1.3.6) you must follow these steps:
  ...
  3. Copy the binaries to /usr/bin using: sudo cp leapdesk* /usr/bin
  4. Correct the permissions and ownership: sudo chown root:wheel /usr/bin/leapdesk*; sudo chmod 555 /usr/bin/leapdesk*
