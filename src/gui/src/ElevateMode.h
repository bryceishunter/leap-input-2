...
// The first, switch-on-desk-switch (SodS), passed through leapdesk-daemon as a
// command line argument to the Leapdesk KVM core, determines if the server restarts
// when switching Windows desktops (e.g. when Windows UAC dialog pops up).
// The second, passed as a boolean flag to leapdesk-daemon over the IPC inside
// kIpcCommandMessage, determines whether Leapdesk KVM should be started with
// elevated privileges.
//
